...
const App = () => {
        ...
        useEffect(() => {
            ...
            console.log("new pdfName", pdfName)
            if (pdfName && (!pdf || (pdf && pdf.name !== pdfName))) {
                setPdf(Pdf(pdfName, null));
                console.log("loading fields...", isPdfReady)
                let formFields = pdfApi.getFormFields();
                setFields(formFields);
                setFieldLists([FieldList(pdfName, formFields)])
            }
